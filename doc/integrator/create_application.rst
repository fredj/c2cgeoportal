...
12. In ``.gitignore`` add the following lines:
...
.. prompt:: bash
    ...
    ENV=<user> make -f <package>.mk user_mk
...
In ``.in`` files, the variable replacement syntax is as follows::
  ...
  ${<configsectionname>:<variablename>}

where the ``<configsectionname>`` is the name of section in the configuration
...
In ``.mako`` files, the variable replacement syntax is as follow::
